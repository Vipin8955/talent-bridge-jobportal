    ...
    const fetchJob = async () => {
      try {
        ...
        if (res.data.success && res.data.job) {
          ...
          setExperienceLevel(j.experienceLevel || 'Mid Level');
          setMinSalary(j.salary?.min || '');
          setMaxSalary(j.salary?.max || '');
          setCurrency(j.salary?.currency || 'INR');
          setSalaryPeriod(j.salary?.period || 'LPA');
          setSkills(j.skills ? j.skills.join(', ') : '');
          setDescription(j.description || '');
          setResponsibilities(
            j.responsibilities ? j.responsibilities.join('\n') : ''
          );
          setRequirements(j.requirements ? j.requirements.join('\n') : '');
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        {/* Status selector */}
        <div className="bg-white rounded-3xl border border-slate-200/80 p-6 sm:p-8 shadow-xs flex items-center justify-between">
          <div>
            <h3 className="text-sm font-bold text-slate-900">Posting Status</h3>
            <p className="text-xs text-slate-500">Active jobs are visible to candidates</p>
          </div>
          <select
            value={status}
            onChange={(e) => setStatus(e.target.value)}
            className="px-4 py-2 rounded-xl border border-slate-200 text-xs font-bold bg-white"
          >
